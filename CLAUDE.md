...
- **Options/presets:** `collectOptions()` reads UI state → options object. Four presets (lossless/web/print/supercompress) map to option combinations. `syncPresetIndicator()` highlights the matching preset when manual settings change. Font subsetting is disabled by default across all presets (under investigation for a rendering issue).
