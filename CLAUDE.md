...
- **Font subsetting safety guards:** Two correctness guards in `font-subset.js` are **not configurable** — they prevent broken output, not trade-offs: (1) cmap-less CIDFontType2 fonts use GID-based subsetting (`hb_subset_input_glyph_set`) because harfbuzz's Unicode path cannot work without a cmap table; (2) already-subsetted simple fonts (`ABCDEF+` prefix) are skipped because their renumbered char codes depend on the embedded cmap. See `font-subset.js` header and `docs/learnings.md`.
...
- **Debug Console:** The Debug Console palette is always created (hidden by default). Open it via the Window menu — no page refresh needed. Adding `?debug` to the URL auto-shows it on load. Debug data (per-pass timing, image skip reasons, per-image conversion details) is always collected (`options.debug` is always `true`). Pass stats objects can include a `_debug` field with additional diagnostics.
...
- **`__BUILD_DATE__`** — Vite `define` in `vite.config.js`, injected as a string (e.g. `"19 Feb 2026"`). Used in the startup warning dialog and About dialog.
- **`__APP_VERSION__`** — Vite `define` in `vite.config.js`, read from `package.json` `version` field. Displayed in About and startup warning dialogs as a clickable link to the GitHub changelog.
